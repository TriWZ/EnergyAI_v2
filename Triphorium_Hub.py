...
# 上传数据
uploaded_data = st.file_uploader("上传建筑或集群数据（CSV，含 timestamp, electricity_kwh 等）", type="csv", key="hub_upload")
...
if uploaded_data:
    ...
    if 'electricity_kwh' in df.columns and 'co2_tons' in df.columns:
        st.metric("累计用电量 (kWh)", f"{df['electricity_kwh'].sum():,.0f}")
        st.metric("累计碳排放 (吨)", f"{df['co2_tons'].sum():,.2f}")
    ...
    st.line_chart(df_monthly.set_index('timestamp')[['electricity_kwh']], height=250, use_container_width=True)
    st.line_chart(df_monthly.set_index('timestamp')[['water_tons']], height=250, use_container_width=True)
    st.line_chart(df_monthly.set_index('timestamp')[['gas_m3']], height=250, use_container_width=True)
    st.line_chart(df_monthly.set_index('timestamp')[['co2_tons']], height=250, use_container_width=True)
